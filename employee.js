...
function doLogin() {
    ...
    if (pin.value.trim() !== '') {
        ...
        try { if (typeof loadNoticesData      === 'function') loadNoticesData(); }      catch(e){}
...
// employee.js 전용 공지사항 초기 데이터
const empDefaultNotices = [
    {
        id: "ntc_1",
        title: "경영혁신형 중소기업(Main-Biz) 인증 갱신",
        desc: "Main-Biz 인증 만료일: 2027.09.13. 유효기간 연장을 위한 경영 평가 및 조달 실적 자료 정리가 필요합니다.",
        date: "2026-06-01",
        dept: "관리팀",
        color: "var(--danger)"
    },
    {
        id: "ntc_2",
        title: "ISO 9001:2015 갱신 주기 도래",
        desc: "품질경영시스템 인증 만료일: 2027.08.22. 인증 대행 실사 일정 확인 및 심사 서류 대조가 요구됩니다.",
        date: "2026-05-28",
        dept: "품질보증부",
        color: "var(--warning)"
    },
    {
        id: "ntc_3",
        title: "춘천시 상수도 공사 장비 납품",
        desc: "다단볼루트펌프 2대 및 부스터 시스템 1조 인수 검사 성적서 발급 완료. 최종 인도가 예정되어 있습니다.",
        date: "2026-05-25",
        dept: "생산본부",
...
function loadNoticesData() {
    const data = localStorage.getItem('batech_notices');
    if (data) {
        try {
        ...
        notices = empDefaultNotices;
        localStorage.setItem('batech_notices', JSON.stringify(empDefaultNotices));
    }
    
    // 발생일 기준으로 내림차순 정렬 (최신순)
...
function updateCalendarEvents() {
    ...
    const events = notices.map(n => ({
        ...
        title: n.title,
        start: n.date,
        backgroundColor: n.color || 'var(--first-color)',
        borderColor: n.color || 'var(--first-color)',
        extendedProps: {
            desc: n.desc,
            dept: n.dept
        }
...
function renderNotices() {
    ...
    if (notices.length === 0) {
        noticeList.innerHTML = '<div style="color: var(--text-color-light); text-align: center; padding: 1rem 0; font-size: var(--small-font-size);">등록된 주요 업무 일정이 없습니다.</div>';
        return;
    }
    ...
    notices.forEach(ntc => {
        ...
        item.style.borderLeftColor = ntc.color || 'var(--first-color)';
        
        item.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                <h4 class="notice-title">${ntc.title}</h4>
                <button class="action-btn" title="삭제" onclick="deleteNotice('${ntc.id}')" style="border: none; background: transparent; padding: 0; width: auto; height: auto; font-size: 1.1rem; color: var(--text-color-light);"><i class="ri-delete-bin-line"></i></button>
            </div>
            <p class="notice-desc">${ntc.desc}</p>
            <div class="notice-date">발생/예정: ${ntc.date} | ${ntc.dept}</div>
        `;
...
function deleteNotice(id) {
    if (confirm('해당 일정을 삭제하시겠습니까?')) {
        ...
        localStorage.setItem('batech_notices', JSON.stringify(notices));
        renderNotices();
        updateCalendarEvents();
    }
}
...
function setupInquiryControls() {
    ...
    // Setup Notice Controls
    const addNoticeBtn = document.getElementById('add-notice-btn');
    const noticeModal = document.getElementById('notice-modal');
    if (addNoticeBtn && noticeModal) {
        addNoticeBtn.onclick = () => {
            document.getElementById('notice-title-input').value = '';
            document.getElementById('notice-desc-input').value = '';
            // 오늘 날짜 기본값
            const today = new Date().toISOString().split('T')[0];
            document.getElementById('notice-date-input').value = today;
            document.getElementById('notice-dept-input').value = '';
            document.getElementById('notice-color-input').value = 'var(--first-color)';
        ...
        document.getElementById('notice-modal-close').onclick = closeNoticeModal;
        document.getElementById('notice-cancel-btn').onclick = closeNoticeModal;
        
        document.getElementById('notice-save-btn').onclick = () => {
            const title = document.getElementById('notice-title-input').value.trim();
            const desc = document.getElementById('notice-desc-input').value.trim();
            const date = document.getElementById('notice-date-input').value;
            const dept = document.getElementById('notice-dept-input').value.trim() || '관리부서';
            ...
            const newNotice = {
                id: 'ntc_' + new Date().getTime(),
                title, desc, date, dept, color
            };
        ...
        if (toggleBtn && listDiv && calDiv) {
            toggleBtn.onclick = () => {
                isCalendarView = !isCalendarView;
                if (isCalendarView) {
                    listDiv.style.display = 'none';
                    calDiv.style.display = 'block';
                    toggleBtn.innerHTML = '<i class="ri-list-check"></i> 목록 보기';
                    
                    if (!fcCalendar && typeof FullCalendar !== 'undefined') {
                        ...
                        fcCalendar = new FullCalendar.Calendar(calendarEl, {
                            ...
                            eventClick: function(info) {
                                // Double check if detail modal exists, else alert
                    ...
                    listDiv.style.display = 'flex'; // notice-list is usually display:flex via flex-direction:column
                    calDiv.style.display = 'none';
                    toggleBtn.innerHTML = '<i class="ri-calendar-line"></i> 달력 보기';
                }
            };
        }
    }
...
async function handleSaveReply() {
    ...
    const btn = document.getElementById('modal-save-reply-btn');
...
async function generateAiResponse(query) {
    ...
    activeSources.forEach(id => {
        if (KnowledgeBase[id]) {
            contextStr += `[문서명: ${KnowledgeBase[id].name}]\n${KnowledgeBase[id].content}\n\n`;
            citations.push(KnowledgeBase[id].name);
        }
    });

    uploadedSources.forEach(doc => {
        if (doc.content) {
            contextStr += `[업로드 문서명: ${doc.name}]\n${doc.content}\n\n`;
    ...
    if (contextStr === '') {
        indicator.remove();
        addMessageBubble('ai', '선택된 소스 문서가 없습니다. 좌측 패널에서 문서를 선택해 주세요.', []);
        return;
    }

    // API Key 검사 로직 삭제 (백엔드에서 처리)

    const prompt = `당신은 펌프 전문 제조업체 (주)비에이텍의 사내 지식 어시스턴트입니다.
아래 제공된 '사내 문서 자료'만을 기반으로 직원의 질문에 정확하고 친절하게 답변하세요.
답변은 읽기 쉽도록 마크다운(굵은 글씨, 목록 등)을 사용해 구조화해 주세요.
만약 문서에 없는 내용이라면 "제공된 문서에서 관련 내용을 찾을 수 없습니다"라고 답변하세요.
